...
void USB_Connect (uint32_t con) {
  ...
  if(con) {
    BITBAND(USB_CONNREG->FIOSET, USB_CONNBIT) = 1;
  } else {
...
//saturnu
//enable bulk in interrupt on nak
//  WrCmdDat(CMD_SET_MODE, DAT_WR_BYTE(INAK_BI));

//test call first interrupt manually and let in transaction interrupt fire the next one automatically
